...
""");
            ...
            var (exitCode, stdout, stderr) = await InvokeConsoleAsync(() => CliApplication.RunAsync([projectFolder, targetWorkingDirectory, "orchestrator.yml", "--json"]));
...
""");
    ...
    public async Task CliApplication_ValidateCommandReportsLoadErrorInJson()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(Path.Combine(projectFolder, "orchestrator.yml"), "name: [");
